...
var MsgErro = Class.create({
	...
	initialize: function(msg)
	{
		...
		this.a = new Element('a',{href: 'javascript:;', onclick:'return false;'}).update('Fechar');
	...
	initialize: function (data, questao)
	{
		...
		for (var a=0; a < data.length; a++)
		{
			...
			for (var b=0; b < data[a].length; b++)
			{
				afastamento = '0;';
				tamanho = '70px;';
				if (data[a][b].tipo == 'normal')
					var classe = 'input_normal'
				else if (data[a][b].tipo == 'pequeno')
				...
				else
				{
					...
					this.divCampos.insert(divQuebra);
...
function permiteContinuar(permite)
{
	...
	if (a)
	{
		if (permite)
		{
			...
			//a = $('SalvaLocal').Pega(nomeSoft, 'automacao_atividade_'+PosicaoAtual.Atividade+'_parte_'+PosicaoAtual.Parte);
		}
		else
		{
